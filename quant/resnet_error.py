...
def inject_error(result, prob, bw=32, bw_hardware=24):
    ...
    # print(f"error inject prob: {prob}")
    return result


class quant_ConvReLU2d_error(nn.Module):
    def __init__(self, in_channels, out_channels, weight, bias, 
                 scale_in, scale_w, scale_out, 
                 prob=None, kernel_size=3, stride=1, padding=1):
        super().__init__()
        ...
        self.quan1 = Quantizer(bit=8, scale=self.scale_w, all_positive=False)
        self.quan2 = Quantizer(bit=8, scale=self.scale_out, all_positive=True)
        self.relu = nn.ReLU(inplace=True)
        self.prob = prob

    def forward(self, x):
        q_input = x / self.scale_in
        q_weight = self.quan1(self.weight) / self.scale_w
        # pdb.set_trace()
        ...
        y = qresult * self.scale_in * self.scale_w + bias
...
class quant_Conv2d_error(nn.Module):
    def __init__(self, in_channels, out_channels, weight, bias, 
                 scale_in, scale_w, scale_out, 
                 zero_point_out, prob, kernel_size=3, stride=1, padding=1):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        ...
        self.quan1 = Quantizer(bit=8, scale=self.scale_w, all_positive=False)
        self.quan2 = Quantizer(bit=8, scale=self.scale_out, zero_point=zero_point_out, all_positive=False)
