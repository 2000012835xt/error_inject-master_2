...
class quant_ConvReLU2d_error(nn.Module):
    ...
    def forward(self, x):
        ...
        q_weight = self.quan1(self.weight) / self.scale1      #q_weight是整数
        # pdb.set_trace()
...
class quant_VGG16_error(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for block in self.features:
            ...
            # pdb.set_trace()
            # if (layer_i==5):
            #     y_integers_err = y_integer 
            # layer_i=layer_i+1
            #if isinstance(block, quant_ConvReLU2d_error):
                #x, y_interger = block(x)
                #self.y_integers_error.append((layer_idx, y_interger))
            #else:
                #x = block(x)
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
